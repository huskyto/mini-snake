...
pub fn standard_snake() -> Result<(), Box<dyn Error>> {
            // SETUP //
    ...
    for d in 0..3 {
        ...
        queue!(out, MoveTo((c_x + d) as u16, c_y as u16), Print('#'))?;
    }

        // APPLE //
    queue!(out, MoveTo(apple.0, apple.1), Print('O'))?;
    ...
    disable_raw_mode()?;
    println!("Final score: {}", deque.len() - 2);
    Ok(())
}
